...
MsgExeNotFound=WARNING - RoomGirl files were not found in selected directory. This patch has to be installed directly to the main game directory in order to work properly.%n%nAre you sure that this directory is correct?
...
jp.MsgIncompatibleModsDetected=警告 - あなたのゲームフォルダで互換性のない改造が検出されました！ あなたは非常に古くなったモッズを使っているか、RoomGirlがの他のゲームのモッズを使っています。 これを修正するには、古いモッズをすべて削除する必要があります。
jp.MsgExeNotFound=警告 - RoomGirlが選択されたディレクトリに見つかりませんでした。 正しく動作させるためには、このパッチをRoomGirlがのメインゲームディレクトリに直接インストールする必要があります。%n%nこのディレクトリが正しいことを確認していますか？
...
MsgPathTooLong=Your game is in a very deeply nested folder, which can cause serious issues or even prevent the game from starting.%n%nPlease move your game folder to a simpler path like for example "D:\Games\RG" and try to run this patch again.
jp.MsgPathTooLong=ゲームは非常に深くネストされたフォルダーにあります。これにより、深刻な問題が発生したり、ゲームが開始できなくなったりする可能性があります。%n%nゲームフォルダを、たとえば「D:\Games\RG」のような単純なパスに移動して、このパッチをもう一度実行してみてください。

MsgPathNonLatin=There are non-latin characters in the game installation path. This can cause issues with mods and the patch.%n%nMove your game to a simple path like "D:\Games\RG" to avoid issues. It is best to use only a-z, A-Z and 0-9.
jp.MsgPathNonLatin=ゲームのインストールパスに非ラテン文字が含まれています。このため、MODやHFで問題が発生することがあります。%n%n問題を避けるために、ゲームを「D:\Games\RG」のようなシンプルなパスに移動してください。a-z、A-Z、0-9のみを使用するのがベストです。

MsgSteamUpdatesMissing=Your game is missing official updates from Steam. Please go to your Steam Library and update RoomGirl before installing this patch.
sc.MsgSteamUpdatesMissing=您的游戏缺少Steam的官方更新。 在安装此补丁之前，请转到您的Steam库并更新RoomGirl。
